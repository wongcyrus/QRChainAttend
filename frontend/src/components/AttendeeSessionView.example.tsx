...
import { StudentSessionView } from './AttendeeSessionView';
 ...
 * 
 * Includes navigation callback when attendee leaves
 */
export function WithLeaveCallback() {
  ...
  const handleLeaveSession = () => {
    console.log('Attendee leaving session');
...
export function WithNextRouter() {
  ...
  export default function SessionPage() {
    ...
    const attendeeId = 'attendee-123'; // From auth context
 ...
 * 
 * Uses authentication context to get attendee ID
 */
...
export function WithErrorBoundary() {
  ...
  export default function SafeSessionView({ sessionId, attendeeId }: {
    sessionId: string;
    attendeeId: string;
  }) {
    return (
...
export function WithLoadingState() {
  ...
  export default function SessionLoader({ qrCode }: { qrCode: string }) {
    ...
    const attendeeId = 'attendee-123'; // From auth
    ...
    if (enrolling) {
      return (
        <div className="enrollment-container">
          ...
          <p>Scan the QR code displayed by your organizer to join the session</p>
          <QRScanner
            isActive={true}
...
export function WithOfflineSupport() {
  ...
  export default function OfflineAwareSession({ sessionId, attendeeId }: {
    sessionId: string;
    attendeeId: string;
  }) {
...
export function WithAnalytics() {
  ...
  export default function TrackedSession({ sessionId, attendeeId }: {
    sessionId: string;
    attendeeId: string;
  }) {
    useEffect(() => {
...
export function MobileAppIntegration() {
  ...
  export default function MobileSessionView({ sessionId, attendeeId }: {
    sessionId: string;
    attendeeId: string;
  }) {
    const webViewUrl = `https://your-app.com/session/${sessionId}?attendeeId=${attendeeId}`;
...
export function WithWebSocket() {
  ...
  export default function RealtimeSession({ sessionId, attendeeId }: {
    sessionId: string;
    attendeeId: string;
  }) {
