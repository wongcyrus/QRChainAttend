/**
 * Attendee Session View Component
 * Feature: prove-present
 * Requirements: 13.1, 13.2, 13.3, 13.5
 * 
 * Main attendee interface for participating in attendance verification:
...
export interface StudentSessionViewProps {
  ...
  /**
   * Attendee ID (from authentication)
   */
  attendeeId: string;

  /**
   * Callback when attendee leaves the session
   */
  onLeaveSession?: () => void;
...
/**
 * Attendee status in the session
 */
...
export function StudentSessionView({
  ...
  /**
   * Fetch attendee status (attendance and holder status)
   */
  const fetchStudentStatus = useCallback(async () => {
    try {
      ...
      // Check if attendee is a holder (would need a dedicated endpoint in real implementation)
  ...
  useEffect(() => {
    ...
    const setupSignalR = async () => {
      try {
        ...
        const negotiateResponse = await fetch(`${apiUrl}/sessions/${sessionId}/attendee/negotiate`, {
          credentials: 'include',
          method: 'POST',
          headers
        });
  ...
  const handleScanSuccess = useCallback(
    (result: ChainScanResponse) => {
      ...
      if (result.success) {
        // Update attendee status based on scan result
        ...
        if (result.newHolder === attendeeId && result.newToken && result.newTokenEtag) {
          // Attendee became the new holder
          ...
          const warningSuffix = result.locationWarning ? ' (Location warning)' : '';
          setSuccessMessage(`You are now the holder! Show your QR code to another attendee.${warningSuffix}`);
        } else {
          setStudentStatus((prev) => ({
            ...prev,
            isHolder: false,
  ...
  if (loading) {
    return (
      <div className={`attendee-session-view ${className}`}>
    ...
    <div className={`attendee-session-view ${className}`}>
      ...
      <div className="header">
        <div className="header-content">
          <h1>Class Session</h1>
          <p className="class-id">{session.eventId}</p>
          {onLeaveSession && (
      ...
      {studentStatus.entryStatus && (
        <div className="attendee-status-card">
          <h2>Your Status</h2>
      ...
      {studentStatus.isHolder && studentStatus.holderToken && (
        <div className="holder-section">
          ...
          <p className="holder-instruction">
            Show this QR code to another attendee to pass the token
          </p>
          <QRDisplay
            ...
            <div className="scan-controls">
              ...
              <p className="scan-instruction">
                {!studentStatus.entryStatus
                  ...
                  ? 'Scan the organizer\'s late entry QR code'
                  : session.earlyLeaveActive
                  ? 'Scan the organizer\'s early leave QR code if you need to leave early'
      ...
      {sessionStarted && !sessionEnded && !studentStatus.entryStatus && (
        <div className="instructions-card">
          ...
          <ol>
            <li>Wait for the organizer to start the entry chain</li>
            <li>When a peer becomes a holder, scan their QR code</li>
            <li>If you become the holder, show your QR code to another attendee</li>
          </ol>
        </div>
      )}
