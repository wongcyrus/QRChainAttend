...
interface SimpleAttendeeViewProps {
  sessionId: string;
  attendeeId: string;
  onLeaveSession?: () => void;
}
...
export function SimpleAttendeeView({ sessionId, attendeeId, onLeaveSession }: SimpleAttendeeViewProps) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (sessionRes.ok) {
        ...
        // Check if attendee is in attendance
        const myAttendance = data.attendance?.find((a: any) => a.attendeeId === attendeeId);
        if (myAttendance) {
          setStatus(prev => ({
            ...prev,
      ...
      // Check if attendee is a holder by looking for active tokens
      try {
        const tokensRes = await fetch(`${apiUrl}/sessions/${sessionId}/tokens/${attendeeId}`, { credentials: 'include', headers });
        if (tokensRes.ok) {
          const tokenData = await tokensRes.json();
          if (tokenData.isHolder && tokenData.token && tokenData.chainId) {
            // Attendee is a holder - generate QR URL
            const baseUrl = process.env.NEXT_PUBLIC_FRONTEND_URL || window.location.origin;
            const holderUrl = `${baseUrl}/attendee?sessionId=${sessionId}&chainId=${tokenData.chainId}&tokenId=${tokenData.token}&type=entry`;
          ...
          // Token endpoint not available or attendee not a holder
          setStatus(prev => ({
            ...prev,
            isHolder: false,
            holderTokenUrl: undefined,
        ...
        // Ignore token fetch errors - attendee is just not a holder
  ...
  const checkForQuestions = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/sessions/${sessionId}/attendee-questions`, {
        credentials: 'include',
        headers
      });
  ...
  useEffect(() => {
    ...
    const reportOnlineStatus = async (isOnline: boolean) => {
      ...
      try {
        const response = await fetch(`${apiUrl}/sessions/${sessionId}/attendee-online`, { credentials: 'include',
          method: 'POST',
          headers,
    ...
    const getNegotiateUrl = async () => {
      ...
      try {
        // Use attendee/negotiate endpoint to connect to dashboard hub for capture events
        const response = await fetch(`${apiUrl}/sessions/${sessionId}/attendee/negotiate`, { credentials: 'include', method: 'POST', headers });
        if (response.ok) {
          const data = await response.json();
          console.log('[SignalR] Attendee negotiate successful');
          return data;
        } else {
          console.log('[SignalR] Attendee negotiate failed:', response.status);
        }
      } catch (err) {
    ...
    const setupSignalR = async () => {
      ...
      if (negotiateData && negotiateData.url) {
        ...
        connection.on('quizQuestion', (question: any) => {
          ...
          // Only show if it's for this attendee
          if (question.attendeeId === attendeeId) {
            setPendingQuestion({
  ...
  // Generate QR code when attendee becomes holder with smooth transition
  useEffect(() => {
  ...
  }, [connectionStatus, status.isHolder, sessionId, attendeeId]);
  ...
  }, [connectionStatus, sessionId, attendeeId]);
      ...
      {status.isHolder && qrCodeUrl && (
        <div style={{
          ...
          <p style={{ marginBottom: '1rem', color: '#856404' }}>
            {timeRemaining > 0 
              ? 'Show this QR code to another attendee' 
      ...
      {!status.entryStatus && (
        <div style={{
          ...
          <p style={{ margin: '0 0 1rem 0', color: '#666' }}>
            You've joined the session successfully. Waiting for the organizer to start attendance...
          </p>
          <div style={{ 
            backgroundColor: '#e3f2fd', 
            padding: '1rem', 
            ...
            <ul style={{ margin: '0.5rem 0 0 0', paddingLeft: '1.5rem' }}>
              ...
              <li style={{ marginBottom: '0.5rem' }}>If you see a QR code: Show it to another attendee to scan</li>
              <li style={{ marginBottom: '0.5rem' }}>If you don't see a QR code: Scan another attendee's QR code with your phone camera</li>
