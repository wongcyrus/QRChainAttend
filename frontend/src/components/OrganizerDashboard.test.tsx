/**
 * Organizer Dashboard Component Tests
 * 
 * Requirements: 12.1, 12.2, 12.3, 12.4
 * 
 * Tests for the Organizer Dashboard component including:
...
import { TeacherDashboard } from './OrganizerDashboard';
...
describe('TeacherDashboard', () => {
  ...
  describe('Chain Status Display - Requirements 12.2, 12.3', () => {
    ...
    test('displays chain details including holder and sequence', async () => {
      ...
      // Check for last holders - use getAllByText since attendee IDs may appear in multiple places
      const studentElements = screen.getAllByText(/attendee-[1-4]/);
  ...
  describe('Attendee List Display - Requirement 12.4', () => {
    test('displays list of students with current status', async () => {
      render(<TeacherDashboard sessionId={mockSessionId} />);
      
      await waitFor(() => {
        expect(screen.getByText('Attendee Attendance (3)')).toBeInTheDocument();
      });
      
      // Check for attendee IDs in the table
      const studentIds = screen.getAllByText(/attendee-[1-3]/);
      expect(studentIds.length).toBeGreaterThanOrEqual(3);
    });

    test('displays correct status for each attendee', async () => {
      render(<TeacherDashboard sessionId={mockSessionId} />);
  ...
  describe('Real-Time Updates - Requirement 12.1', () => {
    test('handles attendance update from SignalR', async () => {
      ...
      // Check that new attendee appears in the table
      await waitFor(() => {
        expect(screen.getByText('Attendee Attendance (4)')).toBeInTheDocument();
      });
      
      // Verify the new attendee ID is in the table
      const studentCells = screen.getAllByText(/attendee-/);
      const hasNewStudent = studentCells.some(cell => cell.textContent === 'attendee-new');
      expect(hasNewStudent).toBe(true);
    });

    test('updates existing attendee record on attendance update', async () => {
      render(<TeacherDashboard sessionId={mockSessionId} />);
  ...
  describe('Chain Updates - Requirement 12.2', () => {
    test('handles chain update from SignalR', async () => {
      ...
      await waitFor(() => {
        const holderCells = screen.getAllByText(/attendee-/);
        const hasUpdatedHolder = holderCells.some(cell => cell.textContent === 'attendee-updated');
