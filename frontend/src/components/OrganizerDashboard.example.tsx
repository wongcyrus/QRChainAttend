/**
 * Organizer Dashboard Component - Example Usage
 * 
 ...
 * 
 * Organizer monitoring multiple sessions with tabs
 */
...
export function ResponsiveDashboard() {
  ...
  return (
    <div className={`responsive-dashboard ${isMobile ? 'mobile' : 'desktop'}`}>
      <h1>{isMobile ? 'Dashboard' : 'Organizer Dashboard'}</h1>
...
export function DashboardWithExport() {
  ...
  const convertToCSV = (attendance: any[]): string => {
    const headers = ['Attendee ID', 'Entry Status', 'Entry Time', 'Exit Verified', 'Exit Time', 'Early Leave', 'Final Status'];
